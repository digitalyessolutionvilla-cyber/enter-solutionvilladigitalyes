...
export default function Blog() {
  useSeo("/blog");
  const allPosts = useBlogPosts(true);
        ...
        <div className="container-custom">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              ...
              <div className="md:flex">
                <div className="md:w-1/2 relative overflow-hidden aspect-video md:aspect-auto">
                  <img src={featured.featured_image ?? ""} alt={featured.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                ...
                <div className="md:w-1/2 p-8 md:p-10 flex flex-col justify-center">
                  <h2 className="text-white font-black text-2xl md:text-3xl mb-4 leading-snug group-hover:text-[#F5D76E] transition-colors">{featured.title}</h2>
                  ...
                  <div className="flex items-center gap-4 text-white/45 text-sm mb-6">
                    ...
                    <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" />{new Date(featured.created_at).toLocaleDateString()}</span>
                  </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(active !== "All" || query !== "" ? filtered : rest).map((post, i) => (
              <motion.article key={post.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.08 }} className="group glass-card rounded-2xl overflow-hidden hover:-translate-y-1.5 transition-all duration-300">
                <div className="relative overflow-hidden aspect-video">
                  ...
                  <span className="absolute top-3 left-3 gradient-brand text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full">{post.category}</span>
                </div>
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between border-t border-white/8 pt-4">
                    <div className="flex items-center gap-3 text-white/45 text-xs">
                      {post.author && <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" />{post.author}</span>}
